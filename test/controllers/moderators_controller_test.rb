...
class ModeratorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create moderator" do
    assert_difference('Moderator.count') do
      post moderators_url, params: { moderator: { email: @moderator.email, name: @moderator.name } }
    end
  ...
  test "should update moderator" do
    patch moderator_url(@moderator), params: { moderator: { email: @moderator.email, name: @moderator.name } }
